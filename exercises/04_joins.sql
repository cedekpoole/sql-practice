...
-- The full orders-to-customers join stays at 830 rows.
...
-- The same pattern for employees with no orders returns 0 rows.
...
-- Alternate path: order_details -> products -> categories.
-- GROUP BY category_id for one row per category.
...
-- Order 10248 becomes 3 product-line rows but remains 1 distinct order.
...
-- Count direct reports while keeping employees with zero.
...
-- Memory hooks
-- Before joining: state the output grain, join path and relationship direction.
-- Join type decides what stays; cardinality decides whether rows multiply.
-- INNER keeps matches. LEFT keeps the left. RIGHT keeps the right. FULL keeps both.
-- CROSS creates every pair: left row count x right row count.
-- With INNER JOIN, written table order usually does not change the result.
-- Join only tables needed for selected columns, filters or relationships.
-- Qualify shared column names: o.order_id.
-- One-to-many joins move rows to the many-side grain.
-- "X with no Y" = LEFT JOIN Y, then WHERE y.primary_key IS NULL.
-- "All X, including zero" = LEFT JOIN and COUNT(y.primary_key), not COUNT(*).
-- Right-table filters in ON limit matches while preserving left rows.
-- The same filters in WHERE can remove NULL rows and undo the LEFT JOIN.
-- COUNT(DISTINCT parent_id) protects parent counts after a detail join.
-- Aggregate each metric at the table grain where that metric naturally lives.
-- GROUP BY the columns identifying what one output row represents.
-- Self-joins use role aliases; chained preservation requires chained LEFT JOINs.
